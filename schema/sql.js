...
var query = {
    ...
    // 临时从后续取评论列表
    jd_goods_havecomment: "select wareId,totalCount,lastId from (SELECT a.wareId,a.totalCount,ifnull(max(c.commentId),0) lastId FROM jd_goods a left join jd_comment c on c.wareId = a.wareId where totalCount>0 group by a.wareId,a.totalCount) a where lastId=0",
    // 已存储的店铺列表
