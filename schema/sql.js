...
var query = {
    ...
    jd_goods_havecomment: "select a.wareId,a.totalCount,0 as lastId from (SELECT a.wareId, max(a.totalCount) totalCount, 0 AS lastId, sum(a.isSync) flag FROM jd_goods a WHERE  totalCount > 20 AND a.wareId NOT IN ( SELECT DISTINCT  wareId FROM  jd_comment ) GROUP BY a.wareId ) a where a.flag=0 order by 2",
    // 已存储的店铺列表
